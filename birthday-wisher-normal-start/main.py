...
if today in birthday_dict.keys():
    bday_person = birthday_dict[today]
    letter = f"./letter_templates/letter_{random.randint(1,3)}.txt"
    with open(letter) as file:
        letter_content = file.read()
    letter_content = letter_content.replace("[NAME]", f"{bday_person["name"]}")
    with smtplib.SMTP("smtp.gmail.com") as connection:
        connection.starttls()
        connection.login(user = my_email,password=password)
        connection.sendmail(from_addr=my_email,to_addrs="timmytimmy456@yahoo.com",msg = f"Subject: Happy Birthday!\n\n{letter_content}")
